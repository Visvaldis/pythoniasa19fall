...
def task3(password):
    ...
    return (6 <= len(password) <= 12
            and any('a' <= letter <= 'z' for letter in password)
            and any('A' <= bigletter <= 'Z' for bigletter in password)
            and any('0' <= number <= '9' for number in password)
...
def task4():
    ...
    # todo: write your code here
    import random
    password = ['']*random.randrange(6,13)
    password[0] = random.choice('#$@')
    password[1] = random.choice('1234567890')
    password[2] = random.choice('qwertyuiopasdfghjklzxcvbnm')
    password[3] = random.choice('QWERTYUIOPASDFGHJKLZXCVBNM')
    superString = '#$@1234567890qwertyuiopasdfghjklzxcvbnmQWERTYUIOPASDFGHJKLZXCVBNM'
    for i in range(4, len(password)):
        password[i] = random.choice(superString) 
